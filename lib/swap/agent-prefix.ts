...
export function buildAgentPrefix(): string {
  ...
  return [
    ...
    "- structural: ONLY salutation lines (\"Dear Hiring Manager,\") and signature lines (\"Sincerely\", \"Best regards\"). A closing SENTENCE — thanks, looking-forward, hoping to hear — is NOT structural: it is aboutYou, and an F06 candidate when it proposes nothing.",
    ...
    "- Failure codes mark FAKE EVIDENCE — sentences that pretend to deliver proof and deliver none. A sentence expressing the applicant's values, standards, criteria for choosing employers, or working practices is a POSITIONING sentence: it delivers no evidence and pretends to none, so it is NEVER a failure and never a technique — emit the booleans and phrases only.",
    "- When in doubt between 'no failure' and a failure: a sentence carrying a number, a named artefact, or a dated event gets no failure. A vague achievement ('significantly improved processing times') is still F01 — topic words are not artefacts.",
